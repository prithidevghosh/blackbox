...
import { informative } from './guard.js';
...
// hits, informative ones lead (D9); among competing fixes the NEWEST in the
// top hit's repo wins (V7/D9) — older ones stay in the store, ranking does
// the invalidation.
export function selectMatches(results, threshold) {
  const above = (results || []).filter((r) => (r.score ?? 0) >= threshold);
  const ranked = above.filter((r) => r.metadata?.source !== 'git');
  const hits = [...ranked.filter(informative), ...ranked.filter((r) => !informative(r))];
  const repo = hits[0]?.metadata?.repo || null;
...
// line and take the newest git result above the same threshold — from the
// failure's own repo when known (a fix for this failure lives where it broke).
export function pickFixFromResults(results, threshold, repo = null) {
  const gits = (results || []).filter(
    (r) => r.metadata?.source === 'git' && (r.score ?? 0) >= threshold && (!repo || r.metadata?.repo === repo)
  );
