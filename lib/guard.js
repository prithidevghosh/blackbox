...
// command pass on semantic score alone. Best git commit above threshold rides
// along as the fix. null = nothing confident to say.
...
// cause, the fix, with session/date. No ANSI — this goes to a model, not a TTY.
export function formatGuardContext({ hit, gitFix }, command = '') {
  ...
  if (gitFix) {
    ...
    if (fix) lines.push(`fix at the time (git, ${dateOf(gitFix)}): ${fix.slice(0, 160)}`);
  }
